...
function renderFretboard(opts) {
  ...
  const H = top + rowH * 5 + (compact ? 26 : 40);
  ...
  // Fret numbers
  const numY = boardTop + boardH + (compact ? 12 : 17);
  for (const f of [...INLAY_FRETS, ...DOUBLE_INLAYS]) {
    if (f > frets) continue;
    const cx = (fretX(f - 1, frets, W - 6, nutX) + fretX(f, frets, W - 6, nutX)) / 2;
    s += `<text x="${cx.toFixed(1)}" y="${numY}" font-size="${compact ? 8.5 : 10.5}" fill="var(--muted)" text-anchor="middle">${f}</text>`;
  }
